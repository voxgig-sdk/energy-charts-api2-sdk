...
// EnergyChartsApi2 SDK retry feature
...
class EnergyChartsApi2RetryFeature extends EnergyChartsApi2BaseFeature
{
    ...
    public function init(EnergyChartsApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EnergyChartsApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EnergyChartsApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EnergyChartsApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
