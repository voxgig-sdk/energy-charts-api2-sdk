...
class PublicPowerEntity
{
    ...
    /**
     * @param PublicPower|array $args PublicPower data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EnergyChartsApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PublicPower|array The current PublicPower data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PublicPower fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EnergyChartsApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PublicPower fields).
     */
    ...
    /**
     * List PublicPower items matching the given filter.
     *
     * @param PublicPowerListMatch|array|null $reqmatch Match filter (any subset
     *   of PublicPower fields) as an assoc-array; PublicPowerListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PublicPower[]|array A list of PublicPower items as assoc-arrays at
     *   the SDK boundary; throws EnergyChartsApi2Error on failure (item-5 convention).
     */
