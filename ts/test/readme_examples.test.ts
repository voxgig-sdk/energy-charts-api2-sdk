...
//     (\`new EnergyChartsApi2SDK(...)\` / \`EnergyChartsApi2SDK.test(...)\` become
//     \`EnergyChartsApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'EnergyChartsApi2SDK'
...
const TEST_SEED = {"entity":{"public_power":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EnergyChartsApi2SDK(...)\` and any
// \`EnergyChartsApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EnergyChartsApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EnergyChartsApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
