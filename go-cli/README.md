# energy-charts-api2-cli

boru-driven command-line client **and** interactive REPL for the EnergyChartsApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/energy-charts-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./energy-charts-api2-cli --help

# 3. Provide credentials once, via the environment
export ENERGY_CHARTS_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./energy-charts-api2-cli list public_power

# 5. Override the API base URL for a single call
ENERGY_CHARTS_API2_BASE=https://api.example.com ./energy-charts-api2-cli list public_power

# 6. No arguments -> interactive REPL
./energy-charts-api2-cli
energy-charts-api2> list public_power
energy-charts-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/energy-charts-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/energy-charts-api2-cli list public_power
   ```
...
Run with no arguments to open a REPL (prompt `energy-charts-api2>`). Each line is
evaluated as its own boru expression:

```text
$ ./energy-charts-api2-cli
energy-charts-api2> list public_power
energy-charts-api2> /help
energy-charts-api2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `public_power`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/energy-charts-api2-cli`. |
...
(`./energy-charts-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
