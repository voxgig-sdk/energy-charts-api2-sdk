...
class PublicPowerEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = EnergyChartsApi2SDK.test(seed, nil)
    seen = base.PublicPower(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.PublicPower(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ENERGY_CHARTS_API2_TEST_PUBLIC_POWER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def public_power_basic_setup(extra)
  ...
  entid_env_raw = ENV["ENERGY_CHARTS_API2_TEST_PUBLIC_POWER_ENTID"]
  ...
  if env["ENERGY_CHARTS_API2_TEST_LIVE"] == "TRUE"
    ...
    client = EnergyChartsApi2SDK.new(Helpers.to_map(merged_opts))
  end
