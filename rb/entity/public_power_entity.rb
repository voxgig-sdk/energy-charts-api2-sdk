...
class PublicPowerEntity
  ...
  def data_set(args)
    if args
      @_data = EnergyChartsApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PublicPower, Hash] the current PublicPower data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EnergyChartsApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PublicPower fields)
  def match_get
  ...
  #
  # @param reqmatch [PublicPowerListMatch, Hash, nil] match filter (any subset of
  #   PublicPower fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<PublicPower>, Array] the matching PublicPower items; raises EnergyChartsApi2Error on failure
