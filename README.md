...
Provides access to energy data.
...
This SDK exposes the API as a small set of **semantic entities** — PublicPower — that you
...
const items = await client.PublicPower().list()
```
...
// publicpowers is an array of PublicPower entities, populated with mock data
// — call publicpowers[0].data() for the record itself
...
```go
client := sdk.Test()
result, err := client.PublicPower(nil).List(
    nil, nil,
)
```
...
// List all publicpowers (returns PublicPowerEntity[] — .data() for the record)
const publicpowers = await client.PublicPower().list()
...
| --- | --- | --- |
| **PublicPower** | The PublicPower entity (list). | `/public_power` |
