# energy-charts-api2-mcp

[MCP](https://modelcontextprotocol.io) server exposing the EnergyChartsApi2 SDK as
two agent tools — `energy-charts-api2_list` and `energy-charts-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/energy-charts-api2-mcp)
make build

# 2. Provide credentials via the environment
export ENERGY_CHARTS_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user energy-charts-api2 \
  -- /absolute/path/to/energy-charts-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./energy-charts-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// energy-charts-api2_list: first page of records
{ "entity": "public_power" }
{ "entity": "public_power", "query": { } }

// energy-charts-api2_load: one record by id
{ "entity": "public_power", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/energy-charts-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user energy-charts-api2 \
     -- "$PWD"/dist/*/energy-charts-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `energy-charts-api2_list` and `energy-charts-api2_load` tools now appear
   in new sessions. Ask the agent to *"list public_power using energy-charts-api2"*
   and it calls `energy-charts-api2_list` with `{"entity":"public_power"}`.
...
```sh
export ENERGY_CHARTS_API2_APIKEY=sk_live_xxx            # API key
export ENERGY_CHARTS_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `energy-charts-api2_list` tool
...
### Call the `energy-charts-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `energy-charts-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `energy-charts-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ENERGY_CHARTS_API2_APIKEY` | API key sent with every request. |
| `ENERGY_CHARTS_API2_BASE` | Optional override of the API base URL. |
...
```sh
./energy-charts-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"energy-charts-api2_load","arguments":{"entity":"public_power","query":{"id":1}}}}'
```
