// Command EnergyChartsApi2-mcp is an MCP server exposing the EnergyChartsApi2
// SDK as MCP tools.
//
// Two tools are registered:
//
//	EnergyChartsApi2_list  — list records of an entity
//	EnergyChartsApi2_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: ENERGY_CHARTS_API2_APIKEY carries the API key and
	// ENERGY_CHARTS_API2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ENERGY_CHARTS_API2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ENERGY_CHARTS_API2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEnergyChartsApi2SDK(opts)
