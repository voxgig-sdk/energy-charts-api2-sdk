...
import (
	...
	omni "github.com/voxgig-sdk/energy-charts-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EnergyChartsApi2Error); is {
				out := map[string]any{
					"name":    "EnergyChartsApi2Error",
...
func omniCtx(arg any, client *sdk.EnergyChartsApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EnergyChartsApi2SDK); is {
			client = live
