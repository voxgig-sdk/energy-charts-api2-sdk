...
func (e *PublicPowerEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *PublicPowerEntity) Deleted() bool {
	return e.deleted
}
...
// argument it returns the current data as an PublicPower; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *PublicPowerEntity) DataTyped(data ...PublicPower) PublicPower {
	if len(data) > 0 {
		return typedFrom[PublicPower](e.Data(asMap(data[0])))
	}
	return typedFrom[PublicPower](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through PublicPower (all fields
// optional at the wire level).
func (e *PublicPowerEntity) MatchTyped(match ...PublicPower) PublicPower {
	if len(match) > 0 {
		return typedFrom[PublicPower](e.Match(asMap(match[0])))
	}
	return typedFrom[PublicPower](e.Match())
}
...
func (e *PublicPowerEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
// PublicPowerListMatch and returns []PublicPower. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *PublicPowerEntity) ListTyped(reqmatch PublicPowerListMatch, ctrl map[string]any) ([]PublicPower, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[PublicPower](res), nil
}
