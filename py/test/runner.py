# EnergyChartsApi2 SDK test runner
...
class EnergyChartsApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EnergyChartsApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EnergyChartsApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EnergyChartsApi2TestRunner.getenv("ENERGY_CHARTS_API2_TEST_LIVE")
        override = EnergyChartsApi2TestRunner.getenv("ENERGY_CHARTS_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EnergyChartsApi2TestRunner.getenv(key)
        ...
        explain = EnergyChartsApi2TestRunner.getenv("ENERGY_CHARTS_API2_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ENERGY_CHARTS_API2_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if EnergyChartsApi2TestRunner._test_control is not None:
            return EnergyChartsApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EnergyChartsApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EnergyChartsApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EnergyChartsApi2TestRunner.load_test_control()
...
def load_env_local():
    EnergyChartsApi2TestRunner.load_env_local()


def env_override(m):
    return EnergyChartsApi2TestRunner.env_override(m)


def entity_data(v):
    return EnergyChartsApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EnergyChartsApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EnergyChartsApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EnergyChartsApi2TestRunner.load_test_control()


def live_client_options():
    return EnergyChartsApi2TestRunner.live_client_options()


def live_delay_ms():
    return EnergyChartsApi2TestRunner.live_delay_ms()
