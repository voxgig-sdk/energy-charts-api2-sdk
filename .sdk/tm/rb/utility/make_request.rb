...
module EnergyChartsApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EnergyChartsApi2Response.new({})
    ...
    if fetch_err
      ...
      response = EnergyChartsApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EnergyChartsApi2Response.new(fetched)
    else
